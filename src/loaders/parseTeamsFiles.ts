...
export async function getTeamsDataFiles(): Promise<FilesData> {
  ...
  //         console.log(
  //           `Teams directory not found at: ${teamsDirectory}. Please ensure the directory exists.`
  //         )
  //         process.exit(1)
  //       }
  //     }
  ...
  for (const file of teamsFile) {
    ...
    // players data
    const players = teamsData.teams.flatMap((team) =>
    ...
    const playerPositions = teamsData.teams.flatMap((team) => {
      return team.players.flatMap((player) => {
        return player.positions.map(
          (position) => `${player.name}_$$_${position}_$$_${team.teamName}`
        )
      })
    })
