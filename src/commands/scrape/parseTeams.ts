...
const { __teamsAnchor, __stadium, __team, __playersTableRows } =
  TEAM_ELEMENT_SELECTORS

export async function getTeams(input: {
  page: PageInstance
  url: string
  leagueName: string
}) {
  ...
  for (const teamLink of teamsLinks) {
    ...
    const stadiumData = await page.$eval(__stadium, (el) =>
    ...
    const teamName = await page.$eval(__team, (el) =>
    ...
    const players: Player = await page.$$eval(__playersTableRows, (rows) =>
      rows.map((row) => {
        const [name, positions, country, shirt, age, height, marketValue] =
