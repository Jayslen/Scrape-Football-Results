...
import { InsertDataCommand } from './insertionCommands.js'
import { getExistingValues, insertRows } from 'src/db/dbStatements.js'
...
export const Insetions: Record<string, InsertionConfig> = {
  leagues: {
    table: 'competitions',
    column: 'league_name',
    dataInserted: 'leagues',
    queriesColumns:
      'INSERT INTO competitions (league_name, league_id, country_id) VALUES ',
    generateQueryValues: () =>
      Array.from(LEAGUES_AVAILABLE).map(
        (league) =>
          `('${
            league.name
          }', UUID_TO_BIN('${randomUUID()}'), (SELECT country_id FROM countries WHERE LOWER(country) = LOWER('${
            league.country
          }'))) `
      ),
  ...
  countries: {
    ...
    generateQueryValues: (countries: Set<string>) =>
      Array.from(countries).map(
        (country) => `('${country}', UUID_TO_BIN('${randomUUID()}'))`
      ),
  ...
  positions: {
    ...
    generateQueryValues: (positions: Set<string>) =>
      Array.from(positions).map(
        (position) => `('${position}', UUID_TO_BIN('${randomUUID()}'))`
      ),
  ...
  stadiums: {
    ...
    queriesColumns:
      'INSERT INTO stadiums (stadium_id, stadium, capacity, inaguration, surface) VALUES ',
    generateQueryValues: () => [],
    getLocalData: () => new Set(),
    useInsertRows: true,
    customLogic: async function (
      self: InsertDataCommand,
      input: { filesStadiums: Set<string>; stadiumsFullData: Stadium[] }
    ) {
      ...
      insertRows({
        ...
        queryValues: Array.from(
          new Set(
            stadiumsFullData
              .filter((stadium) => unsavedStadiums.has(stadium.name))
              .map((stadium) => stadium.name)
          )
        )
          .map((name) =>
            stadiumsFullData.find((stadium) => stadium.name === name)
          )
          .map((stadium) => {
            if (!stadium) return ''
        ...
        ${
          Number.isNaN(parseInt(stadium.yearOpened))
            ? 'NULL'
  ...
  teams: {
    ...
    dataInserted: 'teams',
    queriesColumns:
      'INSERT INTO teams (team_id, name, country_id, stadium_id) VALUES ',
    ...
    customLogic: async function (
      self: InsertDataCommand,
      input: {
        teamsFullData: { name: string; league: string; stadium: string }[]
      ...
      insertRows({
        ...
        queriesColumns:
          'INSERT INTO teams (team_id, name, country_id, stadium_id) VALUES ',
        queryValues: teamsFullData
          .filter((team) => unsavedTeams.has(team.name))
          .map(
            (team) => `(UUID_TO_BIN('${randomUUID()}'),
                    '${team.name.replaceAll("'", "''")}',
                    (SELECT country_id FROM competitions WHERE LOWER(league_name) = LOWER('${
                      team.league
                    }')),
                    (SELECT stadium_id FROM stadiums WHERE LOWER(stadium) = LOWER('${
                      team.stadium
                    }'))
                    )`
          ),
  ...
  players: {
    ...
    queriesColumns:
      'INSERT INTO players (player_id, name, country_id, team_id) VALUES ',
    generateQueryValues: () => [], // Will be handled by custom logic
    getLocalData: () => new Set(), // Will be handled by custom logic
    useInsertRows: true,
    customLogic: async function (
      self: InsertDataCommand,
      input: {
        playersFullData: { name: string; team: string; country: string }[]
      ...
      await insertRows({
        ...
        queriesColumns:
          'INSERT INTO players (player_id, name, country_id, team_id) VALUES ',
        queryValues: playersFullData
          .filter((player) => unsavedPlayers.has(player.name))
          .map(
            (player) => `(UUID_TO_BIN('${randomUUID()}'),
                    '${player.name.replaceAll("'", "''")}',
                    (SELECT country_id FROM countries WHERE LOWER(country) = LOWER('${
                      player.country
                    }')),
  ...
  playersPositions: {
    ...
    customLogic: async function (
      ...
      const unsavedPlayersPositions = new Set(
        Array.from(filesPlayersPosition).filter((pos) => !existingData.has(pos))
      )
      ...
      insertRows({
        ...
        queriesColumns:
          'INSERT INTO player_positions (player_id, position_id) VALUES ',
        queryValues: playersPositionFullData
