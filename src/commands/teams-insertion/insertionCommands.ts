import { insertAll } from '../../db/dbStatements.js'
import { Insetions } from './insertionOptions.js'
...
export class InsertDataCommand {
  ...
  private async executeInsertion(configKey: string, data?: any) {
    const config = Insetions[configKey]
    if (!config) {
  ...
  async insertStadiums(input: {
    filesStadiums: Set<string>
    stadiumsFullData: Stadium[]
  }) {
    await this.executeInsertion('stadiums', input)
  }

  async insertTeams(input: {
  ...
  async insertPlayersPositions(input: {
